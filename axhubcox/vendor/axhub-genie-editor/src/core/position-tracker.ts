...
function getTraversalParent(node: Node | null): Node | null {
  if (!node) return null;
...
function collectScrollTargets(element: Element | null): EventTarget[] {
...
export function createPositionTracker(options: PositionTrackerOptions): PositionTracker {
  ...
  function forceUpdate(force?: boolean): void {
    if (disposer.isDisposed) return;
    cancelRaf();
    if (force) {
      const nextRects = computeRects();
